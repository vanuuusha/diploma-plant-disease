...
По итогам task_15 и task_16 установлено, что CGFM P3+P4+P5 с идентичной-старт инициализацией FiLM функционально тождественна baseline из-за **identity-collapse** (γ ≈ 1, β = 0 + BN-поглощение остаточных возмущений). Лучшая рабочая конфигурация — **P5-only** (task_16, mAP@50 = 0.678, **+2.7 п.п.** к baseline 0.651). Это стало мотивацией для дополнительного исследования (task_19): можно ли пробить identity-collapse и получить работающий CGFM на всех 3 уровнях FPN?
...
|---|---|---|---|---:|---:|---:|
| 0 | (task_15) `cgfm` | identity init | γ=1+tanh(Wc+b), W=0 | 14 | 0.656 | 0.325 |
| 1 | (task_15) `cgfm_v2` | non-identity init | W_γ ~ N(0, 0.05) | 18 | 0.660 | 0.327 |
| 2 | **A. residual** | $F' = F + α(γF + β - F)$, α=0 init | 18 | 0.650 | 0.321 |
| 3 | **B. internal ctx** | context = GAP(backbone-P5), без MobileNetV3 | ≈18 | 0.656 | 0.323 |
| 4 | **C. wide γ** | γ = 0.1 + 1.8·σ(Wc+b) ∈ (0.1, 1.9) | 17 | 0.628 | 0.302 |
| 5 | **D. non-zero β init** | W_β ~ N(0, 0.1) (вместо 0.01) | 17 | 0.640 | 0.313 |
| — | **Микс CGFM+CBAM P5** | CBAM self + FiLM context на одном уровне | 17 | 0.673 | 0.342 |

Для сравнения (итог task_12 + task_16):
- **baseline YOLOv12m aug_diffusion**: 0.651 mAP@50 / 0.365 mAP@50-95
- **CGFM P5-only** (лучшая конфигурация из task_16): 0.678 mAP@50 / 0.369 mAP@50-95

## Ключевые наблюдения

### Вариации A, B (residual + internal context) — близки к cgfm P3+P4+P5

Обе вариации дали результат 0.650–0.656, близкий к baseline (0.651) и базовой P3+P4+P5-конфигурации. Improve над P5-only (0.678) не удалось. Причины:
...
mAP@50 = 0.628 — **ниже baseline** (0.651). Широкий диапазон γ позволяет сети при обучении «обнулить» отдельные каналы (γ→0.1), что даёт слишком сильное возмущение для pretrained-backbone'а. Модель не сходится к стабильному решению.

### Вариация D (non-zero β init std=0.1) — тот же эффект

mAP@50 = 0.640 — ниже baseline, но не так сильно как wide γ. Большая инициализация β (при std=0.1, |β| в начале ~0.05–0.15) ломает baseline signal перед тем, как FiLM-параметры успевают стабилизироваться.

### Микс CGFM + CBAM на P5-only

mAP@50 = **0.673** — выше baseline (+2.2 п.п.), **но немного ниже чистого CGFM-P5-only** (0.678). Комбинация CBAM (self) + FiLM (context) на одном уровне даёт прирост, но **не лучше, чем один CGFM P5-only**. Две модуляции конкурируют: CBAM рекалибрует каналы через самореферентный сигнал, FiLM тут же рекалибрует их через внешний — эффекты частично перекрываются. CGFM P5-only остаётся Pareto-optimal.

## Финальный вывод по task_19

**Ни одна из 5 вариаций не превзошла CGFM P5-only (0.678).** Единственная оптимальная конфигурация CGFM остаётся — **P5-only** из task_16 (+2.7 п.п. над baseline 0.651).
