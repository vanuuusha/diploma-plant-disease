...
| aug_geom → aug_oversample | −1.0 | +1.6 |
| aug_oversample → aug_diffusion | −0.5 | 0.0 |

Эффекты аугментаций на RT-DETR сопоставимы с YOLOv12 на mAP@50 (обе архитектуры дают +2 п.п. на baseline→aug_geom), но на mAP@50-95 поведение расходится: RT-DETR теряет 2.2 п.п. на aug_geom и восстанавливает на aug_oversample, тогда как YOLOv12 выигрывает от всех стадий. Частично это объясняется встроенным global self-attention RT-DETR, который сам по себе даёт часть тех же эффектов (инвариантность к местоположению объекта в сцене), что и геометрические аугментации.
...
- **mAP@50 = 0.61–0.63** несколько ниже ожиданий (0.7–0.8), причина та же, что у YOLOv12: сложность датасета (9 визуально похожих классов, полевые условия съёмки, малый test-срез). Для сравнения архитектур ранжирование информативно: RT-DETR — второй по mAP@50, уступая YOLOv12 во всех четырёх вариантах.
